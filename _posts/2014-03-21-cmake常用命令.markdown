...
# 执行一个外部命令，把输出赋值给cmake变量
execute_process(COMMAND date  '+%Y%m%d' OUTPUT_VARIABLE DATE_TIME OUTPUT_STRIP_TRAILING_WHITESPACE)


# 常用的模块，引用第三方库, 比如curl等等，比如用的FindCURL查找curl库的头文件和链接库，具体参考cmake模块文档的说明
INCLUDE(FindCURL)
...
install(CODE "execute_process(COMMAND \"${CMAKE_COMMAND}\" -E create_symlink
                                ../test.so
                                ${DESTDIR}/usr/lib/test.so.1)
             ")
	     
	     
# 改变 动态链接库的输出名字  libxxx.so,  默认都是有lib开头的可以删掉
    set_target_properties(my_lib PROPERTIES PREFIX "")
    set_target_properties(my_lib PROPERTIES OUTPUT_NAME "new_lib")
Suffix:

    set_target_properties(my_lib PROPERTIES SUFFIX ".so.1")


# CPack 模块打包安装文件RPM
参考CPack模块的说明 https://cmake.org/cmake/help/v2.8.12/cmake.html#module:CPack 


set(CPACK_PACKAGE_NAME test_package)
set(CPACK_PACKAGE_VENDOR test_vendor)
set(CPACK_PACKAGE_VERSION_MAJOR 1)
set(CPACK_PACKAGE_VERSION_MINOR 0)
# set(CPACK_PACKAGE_DESCRIPTION_FILE ${SOURCE_ROOT_DIR}/description.txt)
set(CPACK_PACKAGE_DESCRIPTION_SUMMARY "test package")
set(CPACK_STRIP_FILES true)
set(CPACK_GENERATOR RPM)
set(CPACK_RPM_PRE_INSTALL_SCRIPT_FILE ${SOURCE_ROOT_DIR}/rpm_pre_install.sh)
set(CPACK_RPM_POST_INSTALL_SCRIPT_FILE ${SOURCE_ROOT_DIR}/rpm_post_install.sh)
#set(CPACK_RPM_PRE_UNINSTALL_SCRIPT_FILE ${SOURCE_ROOT_DIR}/rpm_pre_uninstall.sh)
#set(CPACK_RPM_POST_UNINSTALL_SCRIPT_FILE ${SOURCE_ROOT_DIR}/rpm_post_uninstall.sh)
# 如果安装文件到 /etc/httpd目录下和其他的包有冲突，可以忽略目录的创建
set(CPACK_RPM_EXCLUDE_FROM_AUTO_FILELIST_ADDITION /etc/httpd)
...
然后调用下面的命令编译和打包rpm
  mkdir -p build
  cd build
  cmake -DCMAKE_INSTALL_PREFIX=/usr/test -DCMAKE_EXE_LINKER_FLAGS="-Wl,-rpath=/usr/test/lib" ..
  cmake --build .

  cpack -D CPACK_GENERATOR="RPM;TGZ" /path/to/build/tree
```
