...
使用内置的git server 来在widnows 和linux 机器之间同步文件 ，使用多个remote仓库
=========================================================
```text
...
直接在linux上面clone不成功，会收到下面这样错误，windows端不知道为什么发了一个 tcp reset 
   fatal: read error: Connection reset by peer
   fatal: early EOF
   fatal: index-pack failed
但windows本地 clone windows本地dammon服务器是可以的，好像是msys git的一个bug。
把本地clone出来的目录复制到linux，再git pull是可以。
